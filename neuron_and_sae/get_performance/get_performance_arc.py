...
os_data = "../../data/arc/"
os_result = "../result/"
os_performance_result = "../result/arc/"
...
def get_performance(model, tokenizer, masked=None):
    ...
    out_dir = ""
    if masked == None:
        out_dir = f"{os_result}/arc/{args.model_name}/{args.model_name}.jsonl"
        os.makedirs(os.path.dirname(out_dir), exist_ok=True)
        if os.path.exists(out_dir):
            os.remove(out_dir)
    else:
        dir_to_remove = f"{os_result}/arc/{args.model_name}_{masked}"
        ...
        out_dir = f"{os_result}/arc/{args.model_name}_{args.task}_{masked}/{args.model_name}.jsonl"
        os.makedirs(os.path.dirname(out_dir), exist_ok=True)
        if os.path.exists(out_dir):
            os.remove(out_dir)
